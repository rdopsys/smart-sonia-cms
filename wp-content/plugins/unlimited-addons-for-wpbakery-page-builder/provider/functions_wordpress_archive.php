	...
	class UniteFunctionsWPUC_archive{
		...
		public static function getCategoriesHtmlList($catIDs,$strTax = null){
			...
			foreach ( $categories as $category ) {
				...
				if(!empty($link))
					$thelist .= '<a href="' . esc_url( $link ) . '" title="' . esc_attr( sprintf( __( "View all posts in %s", UNLIMITED_ADDONS_TEXTDOMAIN), $category["name"] ) ) . '" ' . $rel . '>' . $catName.'</a>';
				else
					$thelist .= $catName;
		...
		public static function getArrThumbSizes($includeSmall = false){
			...
			foreach($arrWPSizes as $size){
				...
				switch($size){
					...
					case self::THUMB_SMALL:
						...
						break;
					case "ug_big":
						$title = __("Big", UNLIMITED_ADDONS_TEXTDOMAIN);
						break;
				}
			...
			$arrSizes[self::THUMB_LARGE] = __("Large (max width 1024)", UNLIMITED_ADDONS_TEXTDOMAIN);
			$arrSizes[self::THUMB_FULL] = __("Full", UNLIMITED_ADDONS_TEXTDOMAIN);
		...
		public static function writeSettingLanguageFile($filepath){
			...
			foreach($arrText as $text){
				...
				$str .= "_e(\"$text\",\"".UNLIMITED_ADDONS_TEXTDOMAIN."\"); \n";				
			}
			$str .= "?>";
