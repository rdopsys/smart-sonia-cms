...
if( !class_exists('WP_MQTT_Settings') ){
	...
	class WP_MQTT_Settings {
		...
		public function sanitize_event_fields( $event, $is_custom = false ){
			...
			$new['topic'] = empty( $event['topic'] ) ? __( 'topic', 'wp-mqtt' ) : trim( sanitize_text_field( $event['topic'] ) );
		...
		function render_event_fields( $event_name ){
			$event = isset( $this->current_settings[ $event_name ] ) ? $this->current_settings[ $event_name ] : array( 'checkbox'=>false, 'topic'=>__( 'topic', 'wp-mqtt' ), 'message'=>__( 'message', 'wp-mqtt' ) );
			...
			echo '<input id="' . $event_name . '_topic" name="wp_mqtt_settings[' . $event_name . '][topic]" size="30" type="text" value="' . $event['topic'] . '" />&nbsp;';
		...
		function custom_events_callback( $args ){
			...
			if( isset( $this->current_settings['custom_events'] ) && is_array( $this->current_settings['custom_events'] ) ){
				foreach( $this->current_settings['custom_events'] as $key => $event ){
					...
					echo '<input id="custom_event_' . $i . '_topic" name="wp_mqtt_settings[custom_events][' . $i . '][topic]" size="20" type="text" value="' . $event['topic'] . '" />&nbsp;';
