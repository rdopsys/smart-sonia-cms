							...
							array(
								...
								'params'		 => array(
									array(
										...
										'options'	 => array(
											''		 => __( 'Hard resize (generate new image if not existing). Width or height can be empty.<br><span class="cvp-notice">This option does NOT work when using external URL, cloud CDN, Jetpack Photon, etc. for images.</span>', 'content-views-pro' ),
					...
					'params' => array(
						...
						array(
							...
							'params'		 => array(
								array(
									'type'		 => 'html',
									'content'	 => sprintf( '<p class="cvp-notice">%s.</p>', __( 'To set correct format, please check', 'content-views-pro' ) . ' <a target="_blank" href="https://codex.wordpress.org/Formatting_Date_and_Time">' . __( 'this document', 'content-views-pro' ) . '</a>.<br>' . __( 'If the date value is stored in <a href="https://en.wikipedia.org/wiki/Unix_time" target="_blank">Unix time</a>, only date after 1980-01-01 is converted', 'content-views-pro' ) ),
								),
							),
		...
		if ( !empty( $args[ 'params' ][ 0 ][ 'params' ] ) ) {
			...
			$args[ 'params' ][ 0 ][ 'params' ][] = array(
				...
				'params'		 => array(
					array(
						'type'		 => 'checkbox',
						'name'		 => 'scrollable-ffcache',
						'options'	 => PT_CV_Values::yes_no( 'yes', __( 'Prevent layout getting mixed up between devices when using a Cache plugin' ) ),
						'std'		 => '',
						'desc'		 => __( 'Selecting this option will adjust the "Rows Per Slide" value automatically on tablet and mobile devices' ),
					),
				),
				'dependence'	 => array( 'view-type', 'scrollable' ),
			);
