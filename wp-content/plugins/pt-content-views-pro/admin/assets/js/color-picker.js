...
( function ( $ ) {
	...
	var image = 'data:image/png;base64,',
		...
		_deprecated = PT_CV_PUBLIC.wp_before_49;
	...
	$.widget( 'wp.wpColorPicker', $.wp.wpColorPicker, {
		...
		_addListeners: function () {
			...
			self.toggler.on( 'click', function () {
				/* CVP fix self.open() is not defined in WP < 4.0 */
				if ( self.toggler.hasClass( 'wp-picker-open' ) ) {
					if ( typeof self.close === 'function' ) {
						self.close();
					} else {
						// copied in close() of color-picker.js of WP 4.0
						self.element.hide().iris( 'toggle' );
						self.button.addClass( 'hidden' );
						self.toggler.removeClass( 'wp-picker-open' );
						$( 'body' ).off( 'click.wpcolorpicker', self.close );
					}
					...
					if ( typeof self.open === 'function' ) {
						...
						// copied in open() of color-picker.js of WP 4.0
						self.element.show().iris( 'toggle' ).trigger('focus');
	...
	$.widget( 'a8c.iris', $.a8c.iris, {
		_create: function () {
			...
			this.options.alpha = true; /* CVP enables for all elements */
