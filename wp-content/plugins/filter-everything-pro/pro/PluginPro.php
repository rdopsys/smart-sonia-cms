...
//            }
    ...
    private function querySets( $storeKey, $searchKey )
    {
        ...
        if( ! $sets = $container->getParam( $storeKey ) ){
            ...
            if ( flrt_wpml_active() && defined('ICL_LANGUAGE_CODE') && $is_common ) {
    ...
    public function filterSetDefaultFields( $defaultFields, $filterSet )
    {
        ...
        $defaultFields['wp_page_type'] = array(
            ...
            'instructions'  => esc_html__('Specify page(s) where filtering should work', 'filter-everything'),
            'settings'      => true
        );
        ...
        foreach ( $defaultFields as $key => $attributes ){
            ...
            if( $key === 'reset_button_text' ){
                $new_fields['apply_button_page_type'] = array(
                    'type'          => 'Select',
                    'label'         => esc_html__('Where to Show this Filter Set?', 'filter-everything'),
                    ...
                    'instructions'  => esc_html__('Specify where to show this Filter Set in case if this page is different from the page where to filter', 'filter-everything'),
                    'settings'      => true,
                    'skip_view'    => true
                );
