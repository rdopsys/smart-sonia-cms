<div id="astero<?php echo $id; ?>" class="astero<?php echo $service . $radius . $img; ?>" data-<?php echo $plugin_name; ?>='<?php echo $weather; ?>'>
        ...
        <?php if( isset( $custom['search'] ) && $custom['search'] == '1' ): ?>
	        ...
	        $unit_select = $custom['units'] == 'metric' ? '<option value="metric">' . __('&deg;C', ASTERO_SLUG) . '</option><option value="imperial">' . __('&deg;F', ASTERO_SLUG) . '</option>' : '<option value="imperial">' . __('&deg;F', ASTERO_SLUG) . '</option><option value="metric">' . __('&deg;C', ASTERO_SLUG) . '</option>';
	        $select = '<select name="units">' . $unit_select . '</select>';
	        
	        if( isset( $custom['search_text'] ) && $custom['search_text'] != '' ) {
	        	$search_text = str_replace( array('[city]', '[units]'), array( $input, $select ), $custom['search_text'] );

        	} else {
        		$search_text = sprintf(__( 'I want to find the weather for %1$s in %2$s .', ASTERO_SLUG ), $input, $select );
        	}
	        ?>
	        <div class="astero-form center">
	                <form method="post" action="javascript:;">
	                        <p class="margin30"><?php echo $search_text; ?></p>
