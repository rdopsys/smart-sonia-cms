...
class SBI_Admin_Notices {
	...
	public function sbi_license_notices() {
		...
		$user_id                   = $current_user->ID;
		$ignored_on_dashboard_page = get_user_meta( $user_id, 'sbi_ignore_dashboard_license_notice', true );
		...
		if ( empty( $sbi_license ) || ! isset( $sbi_license ) ) {
			return;
		}
		...
		if ( get_option( 'sbi_license_data' ) ) {
			...
			// Call the custom API.
			$request = SBI_HTTP_Request::request( 'GET', $api_url, $args );
		...
		$sbi_interval = round( abs( strtotime( $sbi_todays_date . ' -1 day' ) - strtotime( $sbi_license_expires_date ) ) / 86400 );
		//Is license expired?
		...
		echo '<div class="sbi-admin-notices sbi-license-expired-notice" id="sbi-license-notice">';
		...
		$output = '<span class="sb-notice-icon sb-error-icon">
            ...
            <div class="sb-notice-body">
                <div class="sb-notice-body-content">
                    <h3 class="sb-notice-title">Your Instagram Feed Pro license key has expired!</h3>
                    <p>You are no longer receiving updates that protect you against upcoming Instagram changes</p>
                </div>
                <div class="license-action-btns">
                    <a href="' . $this->get_renew_url() . '" target="_blank" class="sbi-license-btn sbi-btn-blue sbi-notice-btn">Renew License</a>
                    <button class="sbi-license-btn sbi-btn-grey sbi-notice-btn" id="renew-modal-btn">Why Renew?</button>
                    <button class="sbi-license-btn sbi-btn-grey sbi-notice-btn" id="recheck-license-key">
                    ...
                    <div class="sb-why-renew-list-parent">
                        ...
                        <div class="sb-why-renew-list">
                            ...
                            <div class="sb-list-item">
                                ...
                                <p>Photo Captions, Shoppable Feeds, Popup Lightbox, Likes, Comments, Filter Posts, Post Layouts, Hashtag and Tagged Feeds, HighLight and Masonry Layouts, Carousels, and more!</p>
                            </div>
                        </div>
                    </div>
                </div>
