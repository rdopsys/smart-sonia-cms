<div class="sb-onboarding-overlay sb-fs-boss sbi-fb-center-boss" v-if="viewsActive.onboardingCustomizerPopup && iscustomizerScreen">
</div>
<div v-for="tooltip in customizeScreensText.onboarding.tooltips"
     ...
     v-bind:id="'sb-onboarding-tooltip-' + customizeScreensText.onboarding.type + '-' + tooltip.step" class="sbi-fb-popup-inside sb-onboarding-tooltip sbi-fb-source-top" v-bind:class="'sb-onboarding-tooltip-' + tooltip.step" v-bind:data-step="tooltip.step">
	<div class="sbi-fb-popup-cls" @click.prevent.default="onboardingClose()">
	...
	<div class="sb-onboarding-bottom-row">
		...
		<div class="sb-previous-next-wrap">
			<div class="sbi-fb-wrapper">
				<div class="sbi-onboarding-previous sbi-fb-hd-btn sbi-btn-grey sb-button-small sb-button-left-icon" v-bind:data-active="tooltip.step === 1 ? 'false' : 'true'" @click.prevent.default="onboardingPrev">
				...
				<div v-if="customizeScreensText.onboarding.tooltips.length > tooltip.step" class="sbi-onboarding-next sbi-fb-hd-btn sbi-btn-grey sb-button-small sb-button-right-icon" @click.prevent.default="onboardingNext">{{genericText.next}}
					<svg width="6" height="8" viewBox="0 0 6 8" fill="none" xmlns="http://www.w3.org/2000/svg">
						<path d="M1.66656 0L0.726562 0.94L3.7799 4L0.726562 7.06L1.66656 8L5.66656 4L1.66656 0Z" fill="#141B38"/>
					</svg>
				</div>
				<div v-if="customizeScreensText.onboarding.tooltips.length === tooltip.step" class="sbi-onboarding-finish sb-button-small sbi-btn-grey sbi-fb-hd-btn" @click.prevent.default="onboardingClose">{{genericText.finish}}</div>
			</div>
		</div>
	</div>
