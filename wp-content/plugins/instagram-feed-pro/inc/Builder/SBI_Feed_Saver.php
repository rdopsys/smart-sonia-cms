	...
	public function get_feed_settings() {
		if ( $this->is_legacy ) {
			if ( sbi_is_pro_version() ) {
				$instagram_feed_settings = new \SB_Instagram_Settings_Pro( array(), sbi_get_database_settings(), array() );
			} else {
				$instagram_feed_settings = new \SB_Instagram_Settings( array(), sbi_get_database_settings(), array() );
			}
