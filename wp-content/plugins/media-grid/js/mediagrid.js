(function($) {
	...
	$(document).on('click', '.mg_mobile_mode .mg_dd_mobile_filters .mgf_inner', function(e) {
		...
		var $this = $(this);
		if(typeof(mg_dd_toggle_timeout) != 'undefined') {
    ...
    window.mg_matches_per_filter = function(grid_id) {
        ...
        // MGAF
            ...
            $grid.find('.mgaf_sect:not([data-type="range"])').each(function() { 
                ...
                $(this).find('.mgaf_opts_list li, .mgaf_fdd_opt').each(function() {
	...
	$(document).on('keyup', '.mgf_search_form input', function() {
		if(typeof(mg_search_defer) != 'undefined') {
    ...
    $(document).on('click', '.mgi_txt_under', function() {
        ...
        if($inner.is('a')) {
            const target = ($inner.hasAttr('target')) ? $inner.attr('target') : '__self';
	...
	***************************************/
        ...
        if(lcmg.lb_touchswipe) {
            new lc_swiper('#mg_lb_wrap', function(directions, $el, target) {
                ...
                if(
                    ...
                    $target.hasClass('mg_zoomable_img') || $target.parents('.mg_zoomable_img').length ||
                    $target.hasClass('mg_noswipe_el') || $target.parents('.mg_noswipe_el').length
                )  {
                    return false;    
                }

                if(directions.right > 70) {
                    $('.mg_nav_prev > *').trigger('click');
                } 
                else if(directions.left > 70) {
	...
	window.mg_open_item = function(item_id, deeplinked_lb, media_focused_mode) {
		...
		(media_focused_mode) ? $('#mg_lb_wrap').css('display', 'flex') : $('#mg_lb_wrap').css('display', 'block');
	...
	// get item's content
	const mg_get_item_content = function(pid, media_focused_mode, on_item_switch, prepare_it = false) {
		...
		// perform ajax call
            ...
            var data = {
				...
				prev_id : prev_id,
				next_id : next_id,
                mf_mode : (media_focused_mode) ? 1 : 0,
			};
			mg_get_item_ajax = $.post(location.href, data, function(response) {	
				if(static_cache_id) {
            ...
            .fail(function(e) {
                if(e.status) {
                    console.error(e);
                    fill_lightbox('Error retrieving contents. Try again please.');
                }
            });
		}
	...
	window.mg_lb_realtime_actions = function() {
		...
		mg_lb_realtime_actions_intval = setInterval(function() {
            ...
            // zoomable image in medi-focused lb, only if image is bigger - TODO
			...
			if(
				...
				if(mg_lb_lazyloaded) {
					...
					if((real_img_h + player_h) != txt_h) {
						$feat.find('img').not('.mg_lb_zoomed_img').css('height', (txt_h - player_h)).addClass('mg_lb_img_fill');	
					} 
				}
    ...
    $(document).on('click', '.mg_lb_zoom_in_btn', function(e) {
        ...
        if(!$('.mg_lb_zoom_wrap').length) {
            ...
            $wrap.find('.mg_lb_zoom_out_btn').show();
    ...
    $(document).on('click', '.mg_lb_zoom_out_btn', function(e) {
        ...
        if(new_zoom <= 100) {
            ...
            $wrap.find('.mg_lb_zoom_out_btn').hide();  
	...
	$(document).on('click', '.mg_wc_atc_btn:not(.mg_wc_atc_btn_disabled, .mg_wc_atc_btn_acting)', function(e) {
		e.preventDefault();
    ...
    if(lcmg.touch_ol_behav && mg_mobile_device) {
	...
	// text under on link items - emulate a click on the item
	$(document).on('click', '.mg_link .mgi_txt_under', function(e) {
		mg_user_interacted = true;
        
        e.preventDefault();
		$(this).parents('.mg_box').find('a.mg_box_inner')[0].click();
	});
