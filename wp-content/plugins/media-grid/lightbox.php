...
function mg_lightbox($post_id, $prev_item = false, $next_item = false, $media_focused_mode = false) {
        ...
        if($type != 'audio' || ($type == 'audio' && !get_post_meta($post_id, 'mg_soundcloud_url', true))) {
            ...
            $feat_img_code = mg_static::preloader().
                '<div id="mg_lb_feat_img_wrap" class="'. $zoomable_class .'" '. $img_zoom_attr .'>	
                    '. $img_zoom_code .'
                    <img src="'. $feat_img_url .'" alt="'. esc_attr(strip_tags($item_title)) .'" />'.
                '</div>';	
        }
	}
	...
	elseif($type == 'img_gallery') {
        ...
        $featured = '
        <div id="'. $slider_id .'" class="mg_lb_lcms_slider '. $slider_thumbs_class .' '. $slider_img_mode_class .'" style="'. $mf_lb_media_w_css . $cust_slider_h .'" data-extra-nav="'. esc_attr($extra_nav) .'" data-ss-cmd="'. $ss_cmd .'" data-autoplay="'. $ss_autoplay .'">
                    ...
                    if(is_array($attach_video) && isset($attach_video[$a]) && !empty($attach_video[$a])) {
                        if(mg_static::video_embed_url($attach_video[$a]) == 'wrong_url') {
                            ...
                            if(strpos($attach_video[$a], '.youtube.') !== false ) {
                                $featured .= '  
                                <li data-img="'. esc_attr($img_url) .'" data-type="mixed">'. esc_html__('wrong video URL', MG_ML) .'</li>';
                            }
                            else {
                                $featured .= '
                                <li data-img="'. esc_attr($img_url) .'" data-type="video">
                            ...
                            // use data-img for thumbs nav
                            $featured .= '
                            <li data-img="'. esc_attr($img_url) .'" data-type="iframe">
                        ...
                        $featured .= '<li data-img="'. esc_attr($img_url) .'" data-type="image">'. $caption_code .'</li>'; 
                    }
	...
	if(!in_array($cmd_mode, array('inside', 'above', 'ins_hidden', 'round_hidden'))) {
        ...
        (function($) { 
            ...
            $("#mg_lb_top_nav, .mg_side_nav, #mg_top_close").fadeIn(250);  
        })(jQuery);
		</script>';	
	}
	?>
	...
	?>
    <div id="mg_lb_<?php echo $post_id ?>" class="mg_lb_layout mg_layout_<?php echo $layout; ?> mg_lb_<?php echo $cmd_mode ?>_cmd mg_lb_<?php echo $type; ?> <?php echo $navless_class ?>">
      <div>
