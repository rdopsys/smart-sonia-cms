...
class Plugin extends \TotalPollVendors\TotalCore\Foundation\Plugin {
	...
	public function registerProviders() {
		...
		$this->container->share( 'admin.poll.listing', function () {
			return new Admin\Poll\Listing( $this->container->get( 'polls.repository' ), $this->container->get( 'entries.repository' ), $this->container->get( 'log.repository' ), $this->container->get( 'env' ) );
		} );
	...
	public static function onUninstall() {
		...
		foreach ( $sites as $site ):
			...
			if ( $userConsent ):
				...
				endforeach;
				// Delete polls
				$query = new \WP_Query( [ 'post_type' => ['poll', 'poll_preset'], 'post_status' => 'any', 'fields' => 'ids' ] );
