            ...
            jQuery.ajax({
                ...
                success: function () {
                    if (!window['OneSignal'].isPushNotificationsSupported()) {
                        ...
                        return alert(window['TotalPollI18n']['Unfortunately, your browser does not support push notifications.']);
                    }
                    window['OneSignal'].init({
                        appId: _this.SettingsService.settings['notifications']['push']['appId'],
                        autoRegister: true,
                        promptOptions: {
                            actionMessage: window['TotalPollI18n']['Do you want to receive notifications from TotalPoll?'],
                            acceptButtonText: window['TotalPollI18n']['Yes'],
                            cancelButtonText: window['TotalPollI18n']['No']
                        }
                    });
...
(function (TotalPoll) {
    ...
    var FieldsComponent = /** @class */ (function () {
        function FieldsComponent($rootScope, UniqueIdService) {
            ...
            //@PRO
            this.presets.push({
                pattern: /phone|mobile|landline/gi,
                type: 'text',
                validations: {
                    regex: { enabled: true, type: 'match', expression: '^\\(?\\+?([0-9]{1,4})\\)?[-\\. ]?(\\d{10})$', errorMessage: 'Invalid phone number' }
                }
            });
            this.presets.push({
                pattern: /gender|sex/gi,
                type: 'select',
                options: 'male : Male\nfemale : Female',
            });
            this.presets.push({
                pattern: /agree|accept|tos|terms|conditions/gi,
                type: 'checkbox',
                options: 'yes : I have read and I agree with the conditions',
                validations: { filled: { enabled: true } }
            });
            this.presets.push({
                pattern: /country/gi,
                type: 'select',
        ...
        FieldsComponent.prototype.insertBulkFields = function () {
            var _this = this;
            //@PRO
            var fields = this.bulkContent.split("\n");
            fields.forEach(function (field) {
                _this.insertFieldFromPreset(field);
            });
            this.bulkContent = '';
...
(function (TotalPoll) {
    ...
    var ChoiceComponent = /** @class */ (function () {
        ...
        ChoiceComponent.prototype.discover = function (url) {
            ...
            this.processing = true;
            this.embed = null;
            this.EmbedService.fetch(url)
                .then(function (providerResponse) {
                _this.embed = providerResponse;
            })
                .catch(function () {
                if (_this.item[_this.item.type]) {
                    _this.item[_this.item.type].html = null;
                }
            })
                .finally(function () {
                _this.processing = false;
        ...
        ChoiceComponent.prototype.upload = function (context, type) {
            ...
            //@PRO
            this.MediaUploadService.open(type).then(function (attachment) {
                // Get media object
                if (!attachment) {
                    return;
                }
                // Create object
                if (!_this.item[_this.item.type]) {
                    _this.item[_this.item.type] = {};
                }
                // Available sizes
                var availableSizes = attachment.get('sizes');
                // Sizes
                if (availableSizes) {
                    _this.item[_this.item.type].sizes = availableSizes;
                }
                // Id
                _this.item[_this.item.type].attachmentId = attachment.get('id');
                if (context === 'full') {
                    // Full image
                    _this.item[_this.item.type].full = attachment.get('url');
                    // Thumbnail
                    if (_this.item.type === 'image' && availableSizes && availableSizes.thumbnail) {
                        if (!_this.item[_this.item.type].thumbnail || (_this.item[_this.item.type].thumbnail != availableSizes.thumbnail.url && confirm('Override thumbnail URL?'))) {
                            _this.item[_this.item.type].thumbnail = availableSizes ? availableSizes.thumbnail.url : '';
                        }
                    }
                    // Label
                    if (_this.item.label.trim().length === 0) {
                        _this.item.label = attachment.get('title ') || attachment.get('name');
                    }
                    // Delete embed HTML, if any
                    delete _this.item[_this.item.type].html;
                }
...
(function (TotalPoll) {
    ...
    var ChoicesComponent = /** @class */ (function () {
        ...
        ChoicesComponent.prototype.insertBulkChoices = function () {
            var _this = this;
            //@PRO
            var choices = this.bulkContent.split("\n");
            this.collapseChoices();
            choices.forEach(function (choice) {
                if (choice.trim() !== '') {
                    _this.insertChoice({ type: 'text', label: choice });
                }
            });
            this.bulkContent = '';
            this.toggleBulkInput();
        ...
        ChoicesComponent.prototype.handleDrop = function () {
                        ...
                        try {
                            reader.addEventListener('load', function (event) {
                                if (event.target['result']) {
                                    _this.insertChoice({ type: 'html', html: event.target['result'] });
                                }
                            }, false);
                            reader.readAsText(item.getAsFile() || new Blob());
                        }
                        catch (exception) {
                            alert("Something went wrong (" + exception.toString() + "). Retry?");
                        }
                        //@END-PRO
                    }
                    else if (['image', 'video', 'audio'].indexOf(item.type.split('/').shift()) != -1 && item.kind === 'file') {
                        //@PRO
                        // this.MediaUploadService.frame(item.type.split('/').shift());
                        _this.media.open();
                        _this.mediaUploader.addFile([item.getAsFile()]);
                        //@END-PRO
                    }
                });
                setDroppable(false);
            ...
            //@PRO
            this.media.on('insert', function (selection) {
                angular.forEach(selection.toJSON(), function (file) {
                    if (['image', 'video', 'audio'].indexOf(file.type) == -1) {
                        return;
                    }
                    var choice = { type: file.type, label: file.title || file.label };
                    choice[file.type] = { sizes: file.sizes, full: file.url, thumbnail: file.sizes && file.sizes.thumbnail ? file.sizes.thumbnail.url : file.url };
                    _this.insertChoice(choice);
                });
                setDroppable(false);
            });
            //@END-PRO
