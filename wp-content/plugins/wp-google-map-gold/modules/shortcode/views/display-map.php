...
if ( $atts ) {
	foreach ( $atts as $key => $value ) {
		...
		if ( strpos( $key, 'address' ) === 0 ) {
			...
			$geocode               = wp_remote_get( 'https://maps.google.com/maps/api/geocode/json?key=' . $wpgmp_settings['wpgmp_api_key'] . '&address=' . $explode_ads[0] . '&sensor=false' );
...
$map_data['map_options'] = array(
	'center_lat'            => isset($center_lat) ? $center_lat : '',
	'center_lng'            => isset($center_lng) ? $center_lng : '',
	'zoom'                  => $zoom,
	'scroll_wheel'          => $scroll_wheel,
	'fit_bounds'   			=> ( 'false' === $fit_bounds ? false : true ),
...
if ( is_array( $marker_array ) ) {
	if ( $marker_array ) {
		$index = 0;
		foreach ( $marker_array as $key => $marker ) {
			
			$explode_marker = explode( '|', $marker );

			if ( ! empty( $explode_marker[4] ) ) {
				$wpgmp_marker_category_results = isset($all_categories_name[ sanitize_title( $explode_marker[4] ) ]) ? $all_categories_name[ sanitize_title( $explode_marker[4] ) ] : '';
				$cat_id                        = isset($wpgmp_marker_category_results->group_map_id) ? $wpgmp_marker_category_results->group_map_id : '';
				$cat_title                     = isset($wpgmp_marker_category_results->group_map_title) ? $wpgmp_marker_category_results->group_map_title : '';
				$icon                          = isset($wpgmp_marker_category_results->group_marker) ? $wpgmp_marker_category_results->group_marker : '';
			} else {
				$icon = '';
			}
...
if ( is_array( $address_array ) ) {
	if ( $address_array ) {
		foreach ( $address_array as $address ) {
			...
			$geocode         = wp_remote_get( 'https://maps.google.com/maps/api/geocode/json?key=' . $wpgmp_settings['wpgmp_api_key'] . '&address=' . $rm_space_ads . '&sensor=false' );
			if ( ! isset( $geocode->errors ) ) {
				$output = json_decode( $geocode['body'] );
