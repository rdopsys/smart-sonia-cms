	...
	class WPGMP_Model_Tools extends FlipperCode_Model_Base {
		...
		public function clean_database() {
			...
			if ( isset( $data['wpgmp_cleandatabase_tools'] ) ) {
				...
				if ( isset( $data['wpgmp_clean_consent'] ) && $data['wpgmp_clean_consent'] == 'DELETE' ) {
					...
					$response['success'] = esc_html__( 'All locations, maps, routes, categories are removed.', 'wpgmp-google-map' );
				} else {
					$response['error'] = esc_html__( 'Consent could not be verified.', 'wpgmp-google-map' );
				}
			} else {
			...
			if ( isset( $_POST['wpgmp_sampledata_consent'] ) ) {
				...
				if ( isset( $data['wpgmp_sampledata_consent'] ) && $data['wpgmp_sampledata_consent'] == 'YES' ) {
					...
					$sample_data['routes'] = array(
						...
						'Route 1' => array( '#8CAEF2', 1, 10, 'DRIVING', 'METRIC', $location_ids[0], $location_ids[1] ),
						'Route 2' => array( '#8CAEF2', 1, 10, 'DRIVING', 'METRIC', $location_ids[2], $location_ids[3] ),
				...
				// $tables = array( TBL_LOCATION,TBL_GROUPMAP,TBL_MAP,TBL_ROUTES );
				return $response;
			}
		}
