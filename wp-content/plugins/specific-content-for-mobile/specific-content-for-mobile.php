...
define( 'EOS_SCFM_PLUGIN_VERSION','0.1.9.2' );
...
function eos_scfm_post_content_replacement(){
	...
	if( isset( $_SERVER['REQUEST_URI'] ) && false !== strpos( $_SERVER['REQUEST_URI'],'-'.apply_filters( 'scfm_mobile_slug','mobile' ) ) ){
		...
		if( $desktop_post_id > 0 && function_exists( 'is_preview' ) && !is_preview() && !is_customize_preview() ){
...
function eos_scfm_related_desktop_id( $post_id ){
	return absint( get_post_meta( $post_id,apply_filters( 'eos_scfm_desktop_post_id','eos_scfm_desktop_post_id' ),true ) );
}
...
function eos_scfm_related_mobile_id( $post_id ){
	return absint( get_post_meta( $post_id,apply_filters( 'eos_scfm_mobile_post_id','eos_scfm_mobile_post_id' ),true ) );
}
